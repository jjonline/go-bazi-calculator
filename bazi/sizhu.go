...
type TSiZhu struct {
	...
	pDayZhu   *TZhu // 日柱
	pHourZhu  *TZhu // 時柱
	// pHeHuaChong *THeHuaChong // 合化沖 這個暫時沒寫， 後面一定補。
	pSolarDate *TSolarDate // 新歷日期
	pBaziDate  *TBaziDate  // 八字歷日期
	pXiYong    *TXiYong    // 喜用神
}
...
func (m *TSiZhu) init() *TSiZhu {
	...
	// 通過公歷 年月日計算日柱
	nDayGan := m.pDayZhu.genDayGanZhi(m.pSolarDate.GetAllDays()).Gan().Value() // 獲取日幹(日主)
	// 通過小時 獲取時柱
